...
const safeLog = (level, message, meta = {}, correlationId = null) => {
  try {
    ...
    if (level === 'security_auth_failed')
      return secureLogger.security.authenticationFailed(
        meta.ip,
        meta.username,
        meta.reason,
        correlationId
    ...
    try {
      console.error('[auth-logger-fallback]', message, meta && meta.error ? meta.error : '')
    } catch {
      /* no-op */
    }
  }
...
if (!JWT_SECRET) {
  safeLog(
    'warn',
    'JWT_SECRET environment variable is not set; authentication features will be disabled',
    {
      category: 'security',
      type: 'config_warning',
  ...
  try {
    ...
    if (!user) {
      safeLog(
        ...
        { ip: req.ip, username: decoded.email || 'unknown', reason: 'user_not_found' },
        req.correlationId
      )
      return res.status(401).json({
        success: false,
    ...
    if (error.name === 'JsonWebTokenError') {
      safeLog(
        'security_auth_failed',
        'Authentication failed',
        { ip: req.ip, username: 'unknown', reason: 'invalid_token' },
        req.correlationId
      )
      return res.status(401).json({
        success: false,
      ...
      safeLog(
        'security_auth_failed',
        'Authentication failed',
        { ip: req.ip, username: 'unknown', reason: 'token_expired' },
        req.correlationId
      )
      return res.status(401).json({
        success: false,
