    ...
    res.json({
      ...
      data: {
        mp: {
          ...
          // Additional metrics
          completedWorksValue:
            mpSummary?.completedWorksValue || mpSummary?.totalCompletedAmount || 0,
...
const getMPWorks = async (req, res, next) => {
  try {
    ...
    const categoryFilter = category
      ? { workCategory: { $regex: escapeRegex(category), $options: 'i' } }
      : {}
    ...
    if (status === 'recommended' || status === 'all') {
      ...
      if (status === 'recommended') {
        recommendedWorksAggregation.push({ $skip: skip }, { $limit: parseInt(limit, 10) })
      } else {
        // For 'all' status, limit recommended half-page (completed gets the other half with ceil)
        recommendedWorksAggregation.push({ $limit: Math.floor(parseInt(limit, 10) / 2) })
      }
        ...
        const statsAggregation = [
          ...
          ...(has_payments !== undefined
            ? [
                ...
                { $addFields: { hasPayments: { $gt: [{ $size: '$payment_data' }, 0] } } },
                { $match: { hasPayments: has_payments === 'true' || has_payments === '1' } },
              ]
            : []),
          {
            $group: {
    ...
    if (status === 'all') {
      ...
      const completedCountPromise = WorksCompleted.countDocuments({
        ...worksQuery,
        ...categoryFilter,
      })
    ...
    if (has_payments !== undefined && status === 'recommended') {
      ...
      totalCost = filteredWorks.reduce(
        (sum, work) => sum + (work.recommendedAmount || work.estimated_cost || 0),
        0
      )
    ...
    if (houseParam === 'Lok Sabha') {
      ...
      const lsGateSearch = {
        $or: [
          ...
          {
            house: 'Lok Sabha',
            lsTerm: lsSelSearch === 'both' ? { $in: [17, 18] } : parseInt(lsSelSearch, 10),
          },
        ],
      }
  ...
  try {
    ...
    const lsGate =
      mp.house === 'Lok Sabha'
        ? sel === 'both'
          ? { lsTerm: { $in: [17, 18] } }
          : { lsTerm: parseInt(sel, 10) }
        : {}

    // Get expenditure data for this constituency (mp_id or name+constituency) + house/term gating
    const consExpMatch = {
      $and: [
        { $or: [{ mp_id: mp._id.toString() }, { mpName: mp.name, constituency: mp.constituency }] },
        { house: mp.house },
        lsGate,
      ],
    }
...
const getSectorWiseData = async (req, res, next) => {
  try {
    ...
    const lsGateSec =
      house === 'Lok Sabha'
        ? { lsTerm: selSec === 'both' ? { $in: [17, 18] } : parseInt(selSec, 10) }
        : {}
  ...
  try {
    ...
    const lsGateTrend = {
      $or: [
        ...
        {
          house: 'Lok Sabha',
          lsTerm: selTrend === 'both' ? { $in: [17, 18] } : parseInt(selTrend, 10),
        },
      ],
    }
    ...
    const yearlyTrends = await Expenditure.aggregate([
      {
        $match: {
          $and: [{ $or: [{ mp_id: { $in: mpIds } }, { mpName: { $in: mpNames } }] }, lsGateTrend],
        },
      },
    ...
    const worksTrends = await WorksCompleted.aggregate([
      ...
      { $limit: 100000 }, // Higher limit for trend analysis
      {
        $project: {
          year: {
            $year: {
              $ifNull: ['$completedDate', { $dateFromString: { dateString: '$completion_date' } }],
            },
          },
      ...
      terms.map(async t => {
        ...
        const [allocs, exps, wc, wr] = await Promise.all([
          mongoose.connection
            ...
            .countDocuments({ house: 'Lok Sabha', lsTerm: term }),
          mongoose.connection
