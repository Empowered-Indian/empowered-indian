    ...
    const agg = await Summary.aggregate([
      { $match: match },
      {
        $group: {
          ...
          totalAllocated: {
            $sum: {
              $ifNull: ['$allocatedAmount', { $ifNull: ['$totalAllocated', 0] }],
            },
          },
          ...
          totalInProgressPayments: {
            $sum: {
              $ifNull: ['$inProgressPayments', { $ifNull: ['$totalInProgressPayments', 0] }],
            },
          },
            ...
            $cond: [
              ...
              0,
              {
                $cond: [
                  {
                    $or: [
                      { $eq: ['$totalInProgressPayments', null] },
                      { $eq: ['$totalInProgressPayments', 0] },
                    ],
                  },
                  0,
                  {
                    $min: [
                      {
                        $multiply: [
                          {
                            $divide: [
                              { $ifNull: ['$totalInProgressPayments', 0] },
                              '$totalExpenditure',
                            ],
                          },
                          100,
...
const getStateSummary = async (req, res) => {
  try {
    ...
    if (!house) {
      ...
      const stateSummaries = await Summary.find(query)
        .sort({ [sortBy]: order === 'desc' ? -1 : 1 })
        .limit(parseInt(limit))
    ...
    secureLogger.error(
      'Error fetching state summary data',
      {
        category: 'controller',
...
const getMPSummary = async (req, res) => {
  try {
    ...
    const mpSummaries = await Summary.find(query)
      .sort({ [sortBy]: order === 'desc' ? -1 : 1 })
      .limit(parseInt(limit))
    ...
    secureLogger.error(
      'Error fetching MP summary data',
      {
        category: 'controller',
  ...
  try {
    ...
    const avgUtilization =
      totalConstituencies > 0
        ? constituencyData.reduce((sum, c) => sum + (c.utilizationPercentage || 0), 0) /
          totalConstituencies
        : 0
    ...
    secureLogger.error(
      'Error fetching constituency summary data',
      {
        category: 'controller',
