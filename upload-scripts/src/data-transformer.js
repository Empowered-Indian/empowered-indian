...
function transformAllocatedLimit(apiData, house, lsTerm = null) {
  return apiData
    .filter(record => {
      ...
      if (stateName.toLowerCase().includes('total') || mpName.toLowerCase().includes('total')) {
        return false
      }
...
function transformWorksCompleted(apiData, house, lsTerm = null) {
  return apiData
    ...
    .map((record, index) => ({
      ...
      ida: record.IDA_NAME,
      workDescription:
        cleanText(record.WORK_DESCRIPTION) ||
        cleanText(record.ACTIVITY_NAME) ||
      ...
      averageRating:
        record.AVERAGE_RATING !== 'N/A' && record.AVERAGE_RATING !== null
          ? parseFloat(record.AVERAGE_RATING)
          : null,
...
function transformAllData(apiData, options = {}) {
  ...
  results.lok_sabha.allocated_limit = transformAllocatedLimit(
    apiData.lok_sabha.allocated_limit,
    'Lok Sabha',
    lsTerm
  )
  ...
  results.lok_sabha.works_completed = transformWorksCompleted(
    apiData.lok_sabha.works_completed,
    'Lok Sabha',
    lsTerm
  )
  ...
  results.rajya_sabha.allocated_limit = transformAllocatedLimit(
    apiData.rajya_sabha.allocated_limit,
    'Rajya Sabha',
    null
  )
  console.log(
    `✅ Rajya Sabha Allocated Limit: ${results.rajya_sabha.allocated_limit.length} records`
  )

  results.rajya_sabha.expenditure = transformExpenditure(
    apiData.rajya_sabha.expenditure,
    'Rajya Sabha',
    null
  )
  ...
  results.rajya_sabha.works_completed = transformWorksCompleted(
    apiData.rajya_sabha.works_completed,
    'Rajya Sabha',
    null
  )
  console.log(
