...
function isValidRecord(record) {
  ...
  if (
    state.toLowerCase().includes('total') ||
...
async function uploadExpenditures(db, transformedData) {
  ...
  const lsTerms = new Set(
    (transformedData.lok_sabha.expenditure || []).map(r => r.lsTerm).filter(v => v !== undefined)
  )
  for (const term of lsTerms) {
    await collection.deleteMany({ house: 'Lok Sabha', lsTerm: term })
  }
...
async function uploadWorksCompleted(db, transformedData) {
  ...
  if (allWorks.length !== allWorksRaw.length) {
    console.log(
      `ℹ️  Deduplicated works_completed: kept ${allWorks.length} of ${allWorksRaw.length}`
    )
  }
  ...
  try {
    await collection.createIndex(
      { house: 1, lsTerm: 1, state: 1, workId: 1 },
...
async function uploadWorksRecommended(db, transformedData) {
  ...
  try {
    ...
    console.warn(
      'Warning: Could not create unique index for works_recommended:',
      indexError.message
    )
  }
}
...
async function calculateSummaries(db) {
  ...
  const overallSummary = await summariesCollection
    .aggregate([
      {
        $group: {
          _id: null,
                    ...
                    $multiply: [
                      {
                        $divide: [
                          {
                            $ifNull: [
                              { $arrayElemAt: ['$expenditureData.totalExpenditure', 0] },
                              0,
                            ],
                          },
                          '$totalAllocated',
...
async function syncMPLADSDataFromAPI(options = {}) {
  ...
  try {
    ...
    console.log(
      `🔗 Connection pool: ${mongoOptions.minPoolSize}-${mongoOptions.maxPoolSize} connections\n`
    )
    ...
    if (manualCookies) {
      ...
      if (!sessionInitialized) {
        ...
        console.log(
          '💡 You can manually set cookies via environment variable MPLADS_SESSION_COOKIES'
        )
      }
    }
    ...
    const syncStats = {
      ...
      lokSabhaRecords:
        (transformedData.lok_sabha.allocated_limit?.length || 0) +
        (transformedData.lok_sabha.expenditure?.length || 0) +
        (transformedData.lok_sabha.works_completed?.length || 0) +
        (transformedData.lok_sabha.works_recommended?.length || 0),
      rajyaSabhaRecords:
        (transformedData.rajya_sabha.allocated_limit?.length || 0) +
        (transformedData.rajya_sabha.expenditure?.length || 0) +
        (transformedData.rajya_sabha.works_completed?.length || 0) +
        (transformedData.rajya_sabha.works_recommended?.length || 0),
