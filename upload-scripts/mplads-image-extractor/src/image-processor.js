...
export async function processWorkImages(work) {
  ...
  try {
    console.log(
      `🔍 Processing work ${workId} (${mpName || 'Unknown MP'}, ${state || 'Unknown State'})`
    )
    ...
    console.log(
      `📸 Found ${workImages.total} images (${workImages.recommended.length} recommended, ${workImages.completed.length} completed)`
    )

    // Process recommended phase images
    const recommendedImages = await processPhaseImages(
      workId,
      'recommended',
      workImages.recommended
    )
    ...
    const imageData = {
      ...
      summary: {
        ...
        primaryPhase:
          recommendedImages.length > completedImages.length ? 'recommended' : 'completed',
        storageUsed: [...recommendedImages, ...completedImages].reduce(
    ...
    console.log(
      `✅ Successfully processed work ${workId} - ${imageData.summary.totalImages} images (${totalSizeMB} MB)`
    )
...
async function processPhaseImages(workId, phase, attachments) {
  ...
  for (const attachment of attachments) {
    ...
    try {
      ...
      console.log(
        `✅ Uploaded ${fileName} (${Math.round(imageData.processedSize / 1024)}KB) to ${uploadResult.r2Key}`
      )
    } catch (error) {
...
export async function processWorksBatch(works) {
  console.log(
    `🚀 Processing batch of ${works.length} works (${config.processing.maxConcurrentWorkers} concurrent workers)`
  )

  const results = await Promise.allSettled(works.map(work => limit(() => processWorkImages(work))))
  ...
  console.log(
    `📊 Batch complete: ${successful} successful, ${failed} failed | ${totalImages} images | ${(totalStorage / 1024 / 1024).toFixed(2)} MB | ${progressionWorks} progression works`
  )
