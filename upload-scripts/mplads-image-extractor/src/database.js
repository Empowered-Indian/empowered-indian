...
export const WorkCompleted = mongoose.model(
  'WorkCompleted',
  new mongoose.Schema(BaseWorkSchema),
  'works_completed'
)
export const WorkRecommended = mongoose.model(
  'WorkRecommended',
  new mongoose.Schema(BaseWorkSchema),
  'works_recommended'
)
...
export async function getWorksWithImages() {
  const [completed, recommended] = await Promise.all([
    WorkCompleted.find({ hasImage: true }).select(
      'workId mpName state workDescription hasImage imageData'
    ),
    WorkRecommended.find({ hasImage: true }).select(
      'workId mpName state workDescription hasImage imageData'
    ),
  ])
...
export async function updateWorkImages(workId, collection, imageData) {
  ...
  await Model.findOneAndUpdate({ workId }, { $unset: { imageData: '' } })
