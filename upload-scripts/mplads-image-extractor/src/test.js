  ...
  try {
    ...
    for (const work of testWorks) {
      ...
      try {
        ...
        const testImages = [
          ...(workImages.recommended.length > 0
            ? [{ ...workImages.recommended[0], phase: 'recommended' }]
            : []),
          ...(workImages.completed.length > 0
            ? [{ ...workImages.completed[0], phase: 'completed' }]
            : []),
        ]

        for (const testImage of testImages.slice(0, 2)) {
