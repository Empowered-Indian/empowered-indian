...
export class ProgressTracker {
  ...
  logProgress() {
    ...
    console.log(
      `📈 Progress: ${progress.processed}/${progress.total} (${progress.percentage}%) | Success: ${progress.successful} | Failed: ${progress.failed} | Rate: ${progress.rate}/sec | ETA: ${progress.eta}s`
    )
  ...
  complete() {
    ...
    console.log(
      `📊 Final stats: ${finalStats.successful}/${finalStats.total} successful (${((finalStats.successful / finalStats.total) * 100).toFixed(1)}%)`
    )
    console.log(
      `⏱️  Total time: ${finalStats.totalTime}s | Average rate: ${finalStats.avgRate}/sec`
    )
