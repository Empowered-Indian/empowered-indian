...
export async function downloadImageData(attachmentId) {
  ...
  for (let attempt = 1; attempt <= RETRY_CONFIG.maxRetries; attempt++) {
    try {
      console.log(
        attempt > 1
          ? `   🔄 Retry ${attempt}/${RETRY_CONFIG.maxRetries} for attachment ${attachmentId}`
          : ''
      )

      const response = await mpladsClient.post(
      ...
      if (attempt === RETRY_CONFIG.maxRetries) {
        ...
        console.error(
          `❌ Failed to download attachment ${attachmentId} after ${RETRY_CONFIG.maxRetries} attempts:`,
          error.message
        )
        throw error
      }
      ...
      console.log(
        `   ⚠️  Attempt ${attempt} failed: ${error.message} (retrying in ${Math.round(delay / 1000)}s)`
      )
