...
async function main() {
  ...
  console.log(
    `📋 Configuration: ${config.processing.batchSize} batch size, ${config.processing.maxConcurrentWorkers} workers, ${config.processing.rateLimitDelay}ms delay`
  )
  ...
  try {
    ...
    if (worksToProcess.length < allWorks.length) {
      console.log(
        `⏭️  Resuming: ${worksToProcess.length} works remaining (${allWorks.length - worksToProcess.length} already processed)`
      )
    }
    ...
    console.log(
      `📦 Created ${batches.length} batches of up to ${config.processing.batchSize} works each`
    )
      ...
      try {
        ...
        for (const result of batchResults) {
          ...
          if (result.success) {
            ...
            if (work?.state) {
              stats.byState[work.state] =
                (stats.byState[work.state] || 0) + (result.imagesProcessed || 0)
            }
          } else {
            stats.errors.push({
        ...
        if (totalProcessed % config.processing.progressSummaryInterval === 0) {
          const successRate = (
            ((totalProcessed - stats.errors.length) / totalProcessed) *
            100
          ).toFixed(1)
          const avgImagesPerWork =
            stats.totalImages / Math.max(totalProcessed - stats.errors.length, 1)
          console.log(
            `\n📈 PROGRESS SUMMARY: ${totalProcessed}/${worksToProcess.length} works (${((totalProcessed / worksToProcess.length) * 100).toFixed(1)}%)`
          )
          console.log(
            `   ✅ Success rate: ${successRate}% | 📸 Total images: ${stats.totalImages.toLocaleString()}`
          )
          console.log(
            `   💾 Storage used: ${(stats.totalStorage / 1024 / 1024).toFixed(2)} MB | 📊 Avg: ${avgImagesPerWork.toFixed(1)} images/work`
          )
          ...
          console.log(
            `   ⚠️  Errors: ${stats.errors.length} | ⏱️  ETA: ${etaMinutes} minutes remaining`
          )
        }
