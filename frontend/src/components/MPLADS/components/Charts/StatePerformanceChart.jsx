...
const StatePerformanceChart = ({
      ...
      itemStyle: {
        color:
          s.utilizationPercentage > 60
            ? '#10b981'
            : s.utilizationPercentage > 40
              ? '#f59e0b'
              : s.utilizationPercentage > 25
                ? '#f97316'
                : '#ef4444',
      },
    }))
  ...
  useEffect(() => {
    ...
    if (canvasRef.current) {
      if (!chartRef.current) {
        ...
        typeof chartRef.current.getDom === 'function' &&
        chartRef.current.getDom() !== canvasRef.current
      ) {
        try {
          chartRef.current.dispose()
        } catch {
  ...
  const avgUtilization =
    seriesData.top.length > 0
      ? seriesData.top.reduce((sum, s) => sum + (s.utilizationPercentage || 0), 0) /
        seriesData.top.length
      : 0
    ...
    >
      <div className="chart-header">
        <div className="chart-title-section">
          ...
          <InfoTooltip
            content="Top utilization states; bars: utilization, lines: allocated vs spent."
            position="top"
            size="small"
          />
        </div>
          ...
          <div className="highlight-item">
            ...
            <span className="highlight-value">
              {responsive.isSmallMobile && topState.state.length > 12
                ? `${topState.state.slice(0, 12)}...`
                : topState.state}{' '}
              ({(topState.utilizationPercentage || 0).toFixed(1)}%)
            </span>
          </div>
          <div className="highlight-item">
