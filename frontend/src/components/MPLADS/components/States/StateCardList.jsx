    ...
    <div className="state-table-container" role="region" aria-label="States table">
      <div className="state-table">
        <div className="table-header-row">
          {columns.map(col => (
            <div
              ...
              onKeyDown={e => {
                if (e.key === 'Enter' || e.key === ' ') requestSort(col.key)
              }}
              aria-sort={
        ...
        <div className="table-body">
          {rows.length === 0 && <div className="no-data">No states available</div>}
          {rows.map(row => (
            <div
              key={row.id}
              className="table-row"
            ...
            >
              <div
                className={`table-cell align-${columns[0].align}`}
              ...
              <div
                className={`table-cell state-name align-${columns[1].align}`}
                data-label={columns[1].label}
                style={{ width: columns[1].width, minWidth: columns[1].minWidth, overflow: 'wrap' }}
              >
                <div className="state-title">{row.state}</div>
              </div>
              <div
                className={`table-cell align-${columns[2].align}`}
                data-label={columns[2].label}
                style={{ width: columns[2].width, minWidth: columns[2].minWidth }}
              >
                {row.mpCount}
              </div>
              <div
                className={`table-cell align-${columns[3].align}`}
              ...
              >
                {(() => {
                  ...
                  return (
                    <>
                      <div className={`utilization-label ${utilClass}`}>
              ...
              <div
                className={`table-cell align-${columns[6].align}`}
                data-label={columns[6].label}
                style={{ width: columns[6].width, minWidth: columns[6].minWidth }}
              >
                {row.totalWorksCompleted}
              </div>
              <div
                className={`table-cell align-${columns[7].align}`}
                data-label={columns[7].label}
                style={{ width: columns[7].width, minWidth: columns[7].minWidth }}
              >
                {row.recommendedWorksCount}
              </div>
            </div>
          ))}
        </div>
