...
const MPList = () => {
  ...
  const MemoNationalStatsDisplay = useMemo(() => memo(NationalStatsDisplay), [NationalStatsDisplay])
  ...
  const MemoPerformanceInsights = useMemo(() => memo(PerformanceInsights), [PerformanceInsights])
  ...
  if (error) {
    return (
      <div className="mp-list">
        <ErrorDisplay error={error} onRetry={refetch} title="Unable to load MP data" />
      </div>
    )
  }
  ...
  return (
    <div className="mps-page">
      <div className="mps-header">
        ...
        {nationalStats && <MemoNationalStatsDisplay stats={nationalStats} />}
      </div>
      ...
      <div className="mps-content">
        <div className="content-header">
          <h2>All MPs ({formatNumber(totalMPs)})</h2>
          {(filterRange !== 'all' || uiHouse !== 'all' || searchQuery) && (
            <div className="active-filters">
                ...
                <span className="filter-tag">
                  {filterRange === 'high'
                    ...
                    : filterRange === 'medium'
                      ? 'Average Performers'
                      : 'Needs Improvement'}
                  <Button variant="ghost" onClick={() => handleFilterChange('range', 'all')}>
                    ×
                  </Button>
                </span>
              )}
              {uiHouse !== 'all' && (
                <span className="filter-tag">
                  {uiHouse}
                  <Button variant="ghost" onClick={() => handleFilterChange('house', 'all')}>
                    ×
                  </Button>
                </span>
              )}
            ...
            <div className={`mps-${viewMode}`} ref={viewMode === 'grid' ? gridRef : null}>
              ...
              <div className="grid-load-more">
                <div className="page-info">
                  <span>
                    Showing {formatNumber(filteredMPs.length)} of {formatNumber(totalMPs)} MPs
                  </span>
                </div>
                {hasMore && (
                  <Button
