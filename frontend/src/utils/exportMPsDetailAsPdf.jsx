...
const MPDetailDocument = ({ data }) => {
  ...
  const completionColor =
    completionRate >= 80 ? colors.success : completionRate >= 50 ? colors.warning : colors.accent
  const paymentGapColor =
    paymentGapPercentage <= 10
      ? colors.success
      : paymentGapPercentage <= 25
        ? colors.warning
        : colors.accent
        ...
        <View style={styles.content}>
          ...
          <View style={styles.summary}>
            ...
            <Text style={[styles.smallText, { marginBottom: 4 }]}>
              {mp.constituency || 'Constituency'} • {mp.state || 'State'} •{' '}
              {mp.house !== 'Rajya Sabha' ? mp.house : ''}
            </Text>
            ...
            {(mp.name_hi || mp.constituency_hi || mp.state_hi) && (
              <Text style={[styles.smallText, { marginBottom: 8, fontSize: 8, color: '#666' }]}>
                ...
                {mp.constituency_hi && mp.constituency_hi !== mp.constituency
                  ? `${mp.constituency_hi} • `
                  : ''}
              ...
              <View style={styles.analysisColumn}>
                ...
                <View style={styles.analysisMetric}>
                  ...
                  <Text style={styles.analysisMetricValue}>
                    {formatINRCompact(inProgressPayments)}
                  </Text>
            ...
            <View style={styles.chart}>
              ...
              <View style={styles.chartContainer}>
                {yearlyData.map((yearData, i) => {
                  ...
                  const countHeight =
                    maxYearlyCount > 0 ? Math.max(10, (count / maxYearlyCount) * 100) : 10
                  const amountHeight =
                    maxYearlyAmount > 0 ? Math.max(10, (amount / maxYearlyAmount) * 100) : 10
                  return (
              ...
              {completedWorksSliced.map((work, i) => (
                <View key={i} style={styles.workItem}>
                  ...
                  <Text style={styles.workDescription}>
                    {work.workDescription || 'No description'}
                  </Text>
              ...
              {recommendedWorksSliced.map((work, i) => (
                <View key={i} style={styles.workItem}>
                  ...
                  <View style={styles.workMeta}>
                    <Text style={styles.workMetaItem}>
                      Recommended:{' '}
                      {new Date(work.recommendationDate || work.date).toLocaleDateString()}
                    </Text>
                    <Text style={styles.workMetaItem}>
